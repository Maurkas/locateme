...
def save_to_database(name, address, lat, lon):
    ...
    try:
        ...
        Amenities.objects.update_or_create(
            ...
            address=address,
            defaults={
                'type': 'pharmacy',  # Указываем тип удобства
...
async def run_pharmacy_parser(log, stop_flag):
    ...
    async with async_playwright() as p:
        ...
        # Открыть страницу
        url = "https://2gis.ru/astrakhan/search/%D0%90%D0%BF%D1%82%D0%B5%D0%BA%D0%B8?m=48.014771%2C46.346925%2F10.41"
        await page.goto(url)
        ...
        while not stop_flag():
            try:
                next_button = page.query_selector('div._n5hmn94:nth-child(2)')
                if not next_button or '_7q94tr' in next_button.get_attribute('class'):
