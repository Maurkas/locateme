...
def save_to_database(name, address, lat, lon):
    ...
    try:
        ...
        Amenities.objects.update_or_create(
            ...
            address=address,
            defaults={
                'type': 'religious',  # Указываем тип удобства
...
async def run_church_religion_parser(log, stop_flag):
            ...
            try:
                ...
                if 'result' in data and 'items' in data['result']:
                    for item in data['result']['items']:
                        ...
                        if lat and lon:
                            log(f"Название Храмы / Соборы / Церкви: {name}")
                            log(f"Адрес: {address}")
    ...
    async with async_playwright() as p:
        ...
        # Открыть страницу
        url = "https://2gis.ru/astrakhan/search/%D0%A5%D1%80%D0%B0%D0%BC%D1%8B%20%2F%20%D0%A1%D0%BE%D0%B1%D0%BE%D1%80%D1%8B%20%2F%20%D0%A6%D0%B5%D1%80%D0%BA%D0%B2%D0%B8/rubricId/194?m=48.121723%2C46.363155%2F12.13"
        await page.goto(url)
